...
# Usage: ./run.sh [VECTORDB_IP] [TARGET_RPS] [REQUESTS_COUNT] [SCRIPT_TYPE: zipf|normal|no_async_zipf]

# 1. 스크립트 실행 경로를 본인이 위치한 폴더로 고정
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
cd "$SCRIPT_DIR"

VECTORDB_IP=${1:-"163.152.48.208"}      # Qdrant DB가 설치된 Server A IP
TARGET_RPS=${2:-"100.0"}                # Target RPS (요청률)
REQUESTS_COUNT=${3:-"2000"}             # 총 요청 수
SCRIPT_TYPE=${4:-"zipf"}                # 실행 모드 (zipf, normal, no_async_zipf)
...
if [ "$SCRIPT_TYPE" == "zipf" ]; then
  ...
  echo "❌ Error: Invalid SCRIPT_TYPE '$SCRIPT_TYPE'. Use 'zipf', 'normal', or 'no_async_zipf'."
  exit 1
fi
...
echo "   - Target VectorDB: $VECTORDB_IP:6333"
