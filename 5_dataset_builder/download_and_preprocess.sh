...
echo "========================================================================="
echo "  Start Zero-Dependency Dataset & Model Setup (Containerized Method)"
echo "========================================================================="
...
if ! docker image inspect cxl_rag_preprocess:latest >/dev/null 2>&1; then
    echo " Building cxl_rag_preprocess Docker image..."
...
if [ -d "$PROJECT_ROOT/Data/NQ_default" ]; then
    echo "  [Skip] Data/NQ_default already exists. Skipping download."
...
if [ -d "$PROJECT_ROOT/Data/bge-base-en-v1.5" ]; then
    echo "  [Skip] Data/bge-base-en-v1.5 already exists. Skipping download."
...
if [ -d "$PROJECT_ROOT/Data/NQ_default_question_7830" ] && [ -d "$PROJECT_ROOT/Data/NQ_default_embedded_question_7830" ]; then
    echo "  [Skip] Preprocessed datasets (7830 questions & embeddings) already exist."
else
    docker run --rm \
      -u $(id -u):$(id -g) \
      -v "$PROJECT_ROOT":/workspace \
...
if [ -d "$PROJECT_ROOT/Data/Llama3-ChatQA-1.5-8B" ] && [ -f "$PROJECT_ROOT/Data/Llama3-ChatQA-1.5-8B/config.json" ]; then
    echo "  [Skip] Data/Llama3-ChatQA-1.5-8B already exists. Skipping download."
